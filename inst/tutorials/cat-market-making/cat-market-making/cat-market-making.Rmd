...
day0 <- dplyr::as_tibble(cbind(FutContract = names(day0),t(day0))) %>%  
  dplyr::rename(Settle = V2) %>% 
  dplyr::mutate(CalendarMonth =  seq(lubridate::floor_date(dd,unit = "month") + months(1),lubridate::floor_date(dd,unit = "month") + months(14), by = "months"),
...
+ Volume is `1 kbd`. Days in months are specified in the table. 
...
```{r, context = "server"}
df <- shiny::reactiveValues(data = NULL) ## assign it with NULL
    
shiny::observe({df$data <- day0})
    
shiny::observe({ 
  if (!is.null(input$table)) {df$data <- hot_to_r(input$table)} 
  })
    
output$table <- rhandsontable::renderRHandsontable({rhandsontable::rhandsontable(df$data,rowHeaders = NULL)})
```
...
y <- reactive({
  x = df$data
  x %>% dplyr::mutate(Answer = 0)
})

output$table2 <- rhandsontable::renderRHandsontable({rhandsontable::rhandsontable(y(),rowHeaders = NULL)})
