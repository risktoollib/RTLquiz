...
The ask consists in demonstrating your ability the explain the performance of `NANR` as a function of the other variables in using a multivariate regression framework.

In a concise .Rmd (i.e. no `flexdashboards`), follow the workflow in the class note module on multivariate regressions.

1. Extract data from `r from` to `r to` and compute daily `log()` returns.
2. Provide a correlation matrix and/or pairs plot and comment on whether you see potential issues with multicollinearity.
...
5. Make a clear recommendation on what multivariate regression you deem best and the reasons why. Use all the knowledge you gained on regressions up to this point in class.

## Grading

At this point in the class, more differentiation will be applied in grading. This is an advanced class in your learning in your degree and/or professional workplace and you are judged both on absolute and relative standards versus your peers. You are focused on building skills to support your future career success.
...
tibble(Competency=c("Presentation",
           ...
           Measure=c("Professionally rendered presentations. Clean document of unnecessary clutter not relevant to the story. Assume you are presenting to a business leader interested in the story, not how you got to it.  Do not display code unless necessary to enhance your message.",
